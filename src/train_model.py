...
def save_bottlebeck_features():
    ...
    np.save(open('../model_checkpoint/bottleneck_features_train.npy', 'wb'),
    ...
    np.save(open('../model_checkpoint/bottleneck_features_validation.npy', 'wb'),
            bottleneck_features_validation)


def train_top_model():
    train_data = np.load(open('../model_checkpoint/bottleneck_features_train.npy', 'rb'))
    train_labels = np.array(
        [0] * (int(nb_train_samples / 2)) + [1] * (int(nb_train_samples / 2)))

    validation_data = np.load(open('../model_checkpoint/bottleneck_features_validation.npy', 'rb'))
    ...
    model.fit(train_data, train_labels,
              ...
              verbose=0,
              callbacks=[TqdmCallback(), CSVLogger("../model_checkpoint/metrics.csv")])
